...
task :a5book => [:a5, 'tmp/blank_page.png', 'tmp/spacer.png'] do |t|
  ...
  while head < tail do
    ...
    sh "convert -density 600 -size 7016x4961 xc:white #{tmp_img}.t -gravity center -composite -rotate #{head % 2 == 0 ? 270 : 90} #{tmp_img}"
    book << tmp_img
    head += 1
    tail -= 1
  end
  sh <<-_EOS
  convert -density 600 -gravity center -extent 4961x7016#{CONVERT_COMPENSATE} #{book.join ' '} \
...
CONVERT_DITHER = '-ordered-dither h70lines'
